...
class DeviceSerializer(ModelSerializer):
    ...
    def _get_video(self, obj):
        ...
        serialized_data = VideoSerializer(video_objs, many=True,
                                          context={
                                              'playtime': True,
...
class VideoSerializer(ModelSerializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'retrieve' in self.context:
            ...
            self.fields['belongs_to']  = serializers.SerializerMethodField('_get_belongs_to')
        if 'playtime' in self.context:
            self.fields['playtime'] = serializers.SerializerMethodField('_get_playtime')

    def _get_playtime(self, obj):
        playtime_objs = PlayTimeSchedule.objects.filter(video__uuid=obj.uuid,
                                                        device__uuid=self.context.get('device_obj').uuid)
        serialized_data = PlayTimeScheduleSerializer(playtime_objs, many=True).data
        return serialized_data
    ...
    def _get_device(self, obj):
        ...
        serialized_data = DeviceSerializer(device_obj, many=True, context={'playlist': True,
                                                                           'retrieve': True,
                                                                           'video_obj': obj}).data
        return serialized_data
...
class VideoField(serializers.RelatedField):
    ...
    def to_internal_value(self, name):
        ...
        if not skill:
            raise ValueError(
                "{} is not a valid Video".format(id))
        return video
