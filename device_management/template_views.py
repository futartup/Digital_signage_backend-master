    ...
    if request.method == "POST":
        ...
        if resp.status_code == 200:
            ...
            companies_resp = api_call(
                "GET",
                "/api/users/",
                header={"Authorization": "Bearer {}".format(body["access"])},
            )
            body["total_admins"] = len(companies_resp.json())

            # Devices call
            devices_resp = api_call(
                "GET",
                "/api/device/",
                header={"Authorization": "Bearer {}".format(body["access"])},
            )
            body["total_devices"] = len(devices_resp.json())
            body["videos_assigned_devices"] = len(
                [x for x in devices_resp.json() if x["status"] == "True"]
            )

            # Videos
            videos_resp = api_call(
                "GET",
                "/api/video/",
                header={"Authorization": "Bearer {}".format(body["access"])},
            )
            ...
            return render(
                request, "index.html", context={"organization_name": resp.url}
            )
